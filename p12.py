...
dispEpoch = 2
nEpochs = 1000 #should take around a week to run, assuming 10 minutes per epoch
...
startTime = time.time()
prevEpochCost = 1000 #Random value
...
with tf.Session() as sess:
    ...
    for epoch in range(1, nEpochs + 1):
        ...
        if int((avgCost - prevEpochCost) * 10000) == 0: #Check for convergence upto 4 decimal points
            break
        prevEpochCost = avgCost
        startTime = time.time()
